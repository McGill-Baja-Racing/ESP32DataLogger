...
#define BLOCK_SIZE_BYTES   (SAMPLES_PER_BLOCK * INFO_PER_SAMPLE * sizeof(int64_t))
...
static int64_t bufferA[SAMPLES_PER_BLOCK][INFO_PER_SAMPLE];
static int64_t bufferB[SAMPLES_PER_BLOCK][INFO_PER_SAMPLE];
    ...
    while (1) {
        ...
        if (msg.id == ID_NODE1_HEARTBEAT) {
            if (xQueueSend(twai_rx_heartbeat_queue, &msg, 1) != pdPASS) {
                ESP_LOGW(TAG, "Servo Queue FULL — item dropped");
            }


        } 
        // Add to sensor data queue - ADD ALL SENSORS YOU WANT TO LOG!!
        else if(msg.id == 0x0B1 || msg.id == 0x0B2) {
...
static void logger_task(void *arg)
{
    ...
    while (!stop_logging) {
        ...
        // Wait until data received from CAN
        if (xQueueReceive(twai_rx_sample_queue, &msg, pdMS_TO_TICKS(100)) != pdTRUE) {
            continue;
        }
        ...
        //{
            ...
            if (sampleIndex < SAMPLES_PER_BLOCK) {
                ...
                // Select apropriate buffer to write to 
